...
RSpec.describe Hunt, type: :model do
  before (:each) do
    @hunt = Hunt.create(user: User.first, location: Location.first, name: "Test Hunt", start_time: '2018, 1, 1, 12, 00, 00', finish_time: '2018, 1, 1, 15, 00, 00')
  end
  ...
  it 'a new hunt has a default status of pending' do
    hunt = Hunt.create(user: User.first, location: Location.first, start_time: '2018, 1, 1, 12, 00, 00', finish_time: '2018, 1, 1, 15, 00, 00', name: "Test Hunt")
    expect(hunt.status).to eq("pending")
  end

  it 'has a method to display all pending hunts' do
    count = Hunt.pending.count
    Hunt.create(user: User.first, name: "Test Hunt", location: Location.first, start_time: '2018, 1, 1, 12, 00, 00', finish_time: '2018, 1, 1, 15, 00, 00')
    expect(Hunt.pending.count).to eq(count + 1)
  end

  it 'has a method to display all active hunts' do
    count = Hunt.active.count
    Hunt.create(user: User.first, location: Location.first, start_time: '2018, 1, 1, 12, 00, 00', finish_time: '2018, 1, 1, 15, 00, 00', status: "active", name: "Test Hunt")
    expect(Hunt.active.count).to eq(count + 1)
  end

  it 'has a method to display all completed hunts' do
    count = Hunt.completed.count
    Hunt.create(user: User.first, location: Location.first, start_time: '2018, 1, 1, 12, 00, 00', finish_time: '2018, 1, 1, 15, 00, 00', status: "completed", name: "Test Hunt")
